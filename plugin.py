...
<plugin key="AC-ASRplusESP" name="AC Aircon Smart Remote PLUS for ESP" author="MrErwan" version="0.1.1" externallink="https://github.com/Erwanweb/ASR-Plus-ESP.git">
    ...
    <params>
        ...
        <param field="Password" label="AC Brand" width="200px" required="true" default=""/>
...
class BasePlugin:
    ...
    def onMessage(self,Connection,Data):
        ...
        if (Connection == self.httpConnControlInfo):
            ...
            for remoteObject in jsonStatus["System"]:

                # on releve les valeurs

                id = remoteObject["Plugins"]
            ...
            if id > -1:
                ...
                Domoticz.Debug("ESP is connected")
                # ESP Is connected
                if (Devices[1].nValue != 1 or Devices[1].sValue == "0"):
                    Devices[1].Update(nValue = 1,sValue = "100")
                    self.ASRconnexchangedtime = datetime.now()
                # Server SR index
                Devices[2].Update(nValue = 0,sValue = "1")
                # ESP is waitig for order
                if (Devices[3].nValue != 1 or Devices[3].sValue == "0"):
    ...
    def onHeartbeat(self):
        ...
        if self.controlinfotime + timedelta(minutes = 1) <= now:
    ...
    def buildCommandString(self):
        ...
        requestUrl = "/control?cmd=heatpumpir"
