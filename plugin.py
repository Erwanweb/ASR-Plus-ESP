...
class BasePlugin:
    ...
    def checkconnexion(self):
        ...
        Domoticz.Log("checkconnexion called")

        # on met id a -1 pour valider que l'on a bien trouve la telecommande ou l'ESP

        id = -1

        # fetch all the devices from the API and scan for sensors

        devicesAPI = ESPconnectAPI("json")
        if devicesAPI:
            for data in devicesAPI["WiFi"]:  # parse the datas
                id = data["Channel"]
